...
app.get('/', (req, res) => {
  res.json({ 
    message: 'Nagraj Engineering Works API is running!',
    status: 'active',
    endpoints: ['/api/health', '/api/auth/login', '/api/workers', '/api/users']
  });
});
...
if (!MONGODB_URI) {
  ...
  console.error('💡 Please add MONGODB_URI in your hosting platform (Koyeb/Render/Cyclic).');
  process.exit(1); // Stop the server if no database URI
}
...
.catch(err => {
  ...
  console.error('💡 Check your MONGODB_URI value and IP whitelist (0.0.0.0/0) in MongoDB Atlas.');
  process.exit(1);
});
...
const defaultUsers = [
  ...
  { email: 'Admin', password: '358', name: 'Admin User', role: 'super_admin', phone: '', source: 'system' },
  { email: 'CEO', password: '1', name: 'Nagraj CEO', role: 'ceo', phone: '9999999901', source: 'system' },
  { email: 'HR', password: '12', name: 'Nagraj HR', role: 'hr', phone: '9999999902', source: 'system' },
  { email: 'PH', password: '10', name: 'Production Head', role: 'production_head', phone: '9999999903', source: 'system' },
  { email: 'C', password: '20', name: 'Contractor', role: 'contractor', phone: '9999999904', source: 'system' }
];
...
app.listen(PORT, '0.0.0.0', async () => {
  ...
  console.log(`✅ Server ready. Health check: http://localhost:${PORT}/api/health`);
});